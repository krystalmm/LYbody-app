  ...
  describe '#create' do
      ...
      aggregate_failures do
        expect do
          post reservations_path, params: { reservation: {
            user_id: user.id, instructor_id: instructor.id, "start_time(1i)": DateTime.current.year.to_s, "start_time(2i)": (DateTime.current.month + 1).to_s, "start_time(3i)": '01', "start_time(4i)": '17', "start_time(5i)": '00'
          }, format: :js }
    ...
    it 'fails create reservation with invalid datetime' do
      aggregate_failures do
        expect do
          post reservations_path, params: { reservation: {
            user_id: user.id, instructor_id: instructor.id, "start_time(1i)": (DateTime.current.year + 1).to_s, "start_time(2i)": '02', "start_time(3i)": '31', "start_time(4i)": '17', "start_time(5i)": '00'
          }, format: :js }
  ...
  describe '#update' do
    ...
    it 'succeeds update reservation' do
      patch reservation_path(reservation), params: { reservation: {
        user_id: user.id, instructor_id: instructor.id, "start_time(1i)": DateTime.current.year.to_s, "start_time(2i)": (DateTime.current.month + 1).to_s, "start_time(3i)": '10', "start_time(4i)": '17', "start_time(5i)": '00'
      } }
      aggregate_failures do
